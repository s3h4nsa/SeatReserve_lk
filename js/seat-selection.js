...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function generateSeatingChart() {
    ...
    for (let i = 0; i < rows; i++) {
      ...
      for (let j = 0; j < seatsPerRow; j++) {
        ...
        // Add keyboard support for seat selection:
  ...
  function setupCountdownTimer() {
    ...
    let timeLeft = Number.parseInt(sessionStorage.getItem("seatSelectionTimer")) || 10 * 60 // 10 minutes in seconds
    ...
    function updateTimer() {
      ...
      sessionStorage.setItem("seatSelectionTimer", timeLeft.toString())
    ...
    window.addEventListener("beforeunload", () => {
      // Only clear if navigating to a page other than checkout
      if (!window.location.href.includes("checkout.html")) {
        sessionStorage.removeItem("seatSelectionTimer")
      }
    })
  }
