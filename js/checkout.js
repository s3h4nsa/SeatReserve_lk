document.addEventListener("DOMContentLoaded", () => {
  ...
  const moviesData = {
    ...
    3: {
      ...
      trailerUrl: "https://www.youtube.com/watch?v=ezQreKpVO9g&pp=ygURa2FzaSB3YXNpIHRyYWlsZXI%3D",
      imdbRating: "4.5",
      adultPrice: 550,
      childPrice: 350,
    },
    4: {
      ...
      trailerUrl: "https://www.youtube.com/watch?v=uYPbbksJxIg&t=14s&pp=ygUSb3BwZW5oaW1lciB0cmFpbGVy",
      imdbRating: "8.9",
      adultPrice: 700,
      childPrice: 500,
    },
    5: {
      ...
      image: "assets/poster/Dune-Two.jpg",
      ...
      trailerUrl: "https://www.youtube.com/watch?v=Way9Dexny3w&pp=ygUTZHVuZSBwYXJ0IDIgdHJhaWxlctIHCQm9AIO1pN6f1A%3D%3D",
      imdbRating: "8.5",
      adultPrice: 700,
      childPrice: 500,
    },
  ...
  function init() {
    ...
    applyPromoBtn.addEventListener("keydown", (e) => {
    ...
    // Mobile menu toggle - use shared function if available
  ...
  function completeCheckout() {
    try {
      if (validatePaymentForm()) {
        // Show loading state
        const completePurchaseBtn = document.getElementById("complete-purchase")
        const originalText = completePurchaseBtn.textContent
        completePurchaseBtn.disabled = true
        completePurchaseBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Processing...'
        ...
        setTimeout(() => {
          ...
          completePurchaseBtn.textContent = originalText
        }, 1500)
      }
    } catch (error) {
      ...
      errorMessage.textContent = "There was an error processing your payment. Please try again."
  ...
  // Form Validation
  // Replace the form validation functions with calls to the shared module

  // ---------- Form Validation ----------
  function validateContactForm() {
    // Use the shared FormValidator if available
    if (window.SeatReserve && window.SeatReserve.FormValidator) {
      return window.SeatReserve.FormValidator.validateForm(document.getElementById("contact-form"), {
        "first-name": {
          required: true,
      ...
      // Fallback to original validation if shared module isn't available
      let isValid = true
  ...
  function validatePaymentForm() {
    // Use the shared FormValidator if available
    if (window.SeatReserve && window.SeatReserve.FormValidator) {
      return window.SeatReserve.FormValidator.validateForm(document.getElementById("payment-form"), {
        "card-number": {
          required: true,
          requiredMessage: "Card number is required",
          validate: (value) => value.replace(/\s/g, "").length === 16,
          validateMessage: "Please enter a valid 16-digit card number",
        },
        "expiry-date": {
          required: true,
          requiredMessage: "Expiry date is required",
          pattern: /^\d{2}\/\d{2}$/,
          patternMessage: "Please use MM/YY format",
          validate: (value) => {
          ...
          validateMessage: "Card has expired or has an invalid date",
        },
        cvv: {
          required: true,
      ...
      if (!expiryValue) {
        ...
        expiryDateInput.setAttribute("aria-invalid", "false")
      ...
      if (!cardNameInput.value.trim()) {
        ...
        cardNameInput.setAttribute("aria-invalid", "false")
      }
  ...
  // Use shared input formatting functions
  function setupInputFormatting() {
    if (window.SeatReserve && window.SeatReserve.FormValidator) {
      // Use shared formatting functions
      cardNumberInput.addEventListener("input", window.SeatReserve.FormValidator.formatCardNumber)
      expiryDateInput.addEventListener("input", window.SeatReserve.FormValidator.formatExpiryDate)
      cvvInput.addEventListener("input", window.SeatReserve.FormValidator.validateNumericInput)
      phoneInput.addEventListener("input", window.SeatReserve.FormValidator.formatPhoneNumber)
    } else {
      // Fallback to original formatting functions
  ...
  function updateOrderSummary() {
    ...
    if (currentBooking.movie.image) {
      ...
      moviePosterElement.onerror = function () {
        this.src = "/placeholder.svg?height=120&width=80"
        this.onerror = null
      }
    }
  ...
  // Improved trailer playback functionality
  function setupTrailerPlayback() {
    ...
    playTrailerBtn.addEventListener("click", () => {
      ...
      const movie = window.SeatReserveData?.getMovie(movieId) || moviesData[movieId]
      ...
      if (embedUrl.includes("watch?v=")) {
        const videoId = new URLSearchParams(embedUrl.split("?")[1]).get("v")
        embedUrl = `https://www.youtube.com/embed/${videoId}`
      }

      trailerModal.innerHTML = `
        <div class="trailer-modal-content">
          <span class="close-trailer" aria-label="Close trailer">&times;</span>
          <iframe width="100%" height="100%" src="${embedUrl}" 
            frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" 
            allowfullscreen title="${movie.title} trailer"></iframe>
        </div>
      `
